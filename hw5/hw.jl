...
function step!(::Newton, p::Polynom, xᵢ, step_size)
    _, x̃ = xᵢ
    dp = p'
    x̃, x̃ - step_size*p(x̃)/dp(x̃)
end

function step!(::Secant, p::Polynom, xᵢ, step_size)
    x, x̃ = xᵢ
    dpx = (p(x) - p(x̃))/(x - x̃)
    x̃, x̃ - step_size*p(x̃)/dpx
end

function step!(::Bisection, p::Polynom, xᵢ, step_size)
    x, x̃ = xᵢ
    midpoint = (x + x̃)/2
